...
export default function Banner() {
  ...
  return (
    <section className="bg-[url('https://raw.githubusercontent.com/prebuiltui/prebuiltui/main/assets/hero/gridBackground.png')] w-full bg-no-repeat bg-cover bg-center text-sm pb-20 md:pb-44 pt-16 md:pt-32 px-4 sm:px-6 md:px-12 lg:px-20 min-h-screen flex flex-col justify-center">
      <div className="flex items-center gap-1 bg-white/90 backdrop-blur-sm border border-indigo-200 hover:border-indigo-300 rounded-full w-fit max-w-[85%] mx-auto px-3 sm:px-4 py-2 mt-8 md:mt-12 shadow-md">
        <span className="text-xs xs:text-sm sm:text-sm md:text-base text-slate-700 font-medium">Dernières nouveautés? </span>
        <button 
          onClick={() => setIsNewsletterOpen(true)}
          className="flex items-center gap-1 font-semibold text-indigo-600 hover:text-indigo-700 transition-colors"
        >
          <span className="text-xs xs:text-xs sm:text-sm md:text-xs">En savoir plus</span>
      ...
      <h5 className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-bold max-w-[95%] sm:max-w-[500px] md:max-w-[1200px] text-center mx-auto mt-8 xs:mt-12 sm:mt-16 leading-tight bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900 bg-clip-text text-transparent px-2">
        Obtenez un site web qui attire<br />vos clients dès aujourd'hui
      </h5>

      <p className="text-base xs:text-lg sm:text-xl md:text-lg mx-auto max-w-[95%] xs:max-w-[90%] sm:max-w-2xl md:max-w-3xl text-center mt-8 xs:mt-10 sm:mt-14 text-slate-700 leading-relaxed px-2">
      ...
      <div className="mx-auto w-full flex items-center justify-center mt-12 xs:mt-16 sm:mt-20 px-4">
        <button 
          ...
          className="bg-slate-800 hover:bg-black text-white px-6 xs:px-8 sm:px-8 py-3 xs:py-4 sm:py-4 rounded-full font-medium transition text-base xs:text-lg sm:text-lg"
        >
